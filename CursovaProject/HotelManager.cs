  ...
  internal class HotelManager
  {
    public ObservableCollection<Hotel> CreateHotelsObjectsBasedOnSavedData(string[] pathsToData, string[] names)
    {
      ...
      for (int i = 0; i < pathsToData.Length; i++)
      {
        ...
        //int availableStandartRooms = Convert.ToInt32(data[7].Substring(data[7].IndexOf(':') + 1));
        //int availableSuperiorRooms = Convert.ToInt32(data[8].Substring(data[8].IndexOf(':') + 1));
        //int availablePresidentRooms = Convert.ToInt32(data[9].Substring(data[9].IndexOf(':') + 1));
    ...
    private void CreateRoomsBasedOnData(string pathToHotelsDatabase, string hotelName, Hotel hotel)
    {
      ...
      for (int i = 0; i < directories.Length; i++)
      {
        ...
        DateTime dateOut = Convert.ToDateTime(data[3].Substring(data[3].IndexOf(':') + 1));
                hotel.RegisterRoom(GetRoomType(roomType),dateIn, dateOut, roomNumber);
    ...
    private void GetSavedResidents(string pathToHotelDatabase, int roomNumber, Hotel hotel)
    {
      ...
      if (File.Exists(pathToHotelDatabase + "/" + roomNumber + "/" + "residents.txt"))
      {
        ...
        for (int i = 0; i < data.Length; i += 5)
        {
          ...
          int passportSeries = Convert.ToInt32(data[i + 3].Substring(data[i + 3].LastIndexOf(":") + 1));
          int passportNumber = Convert.ToInt32(data[i + 4].Substring(data[i + 4].LastIndexOf(":") + 1));

          persons.Add(new Person(name, secondName, surname, passportSeries, passportNumber));
        }
